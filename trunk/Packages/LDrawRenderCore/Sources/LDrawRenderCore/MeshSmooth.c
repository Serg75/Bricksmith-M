...
#include <LDrawRenderCore/MeshSmooth.h>
...
struct RTree_node * index_vertices_recursive(struct Vertex ** begin, struct Vertex ** end, int depth)
{
	...
	else
	{
		...
		if (depth > 0)
			quickSort_n(begin, 0, count - 1, depth % 3);
		int split = count / 2;
...
// is colinear with the edge DB (as part of ADB), and this is bad.  This is bad 
