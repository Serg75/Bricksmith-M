...
// Concrete subclass registered by the renderer (LDrawTextureMTL or
// LDrawTextureGL). Used by +textureClass for GPU-agnostic instantiation.
static Class LDrawTexture_registeredClass = Nil;

//---------- textureClass --------------------------------------------[static]--
//
// Purpose:		Returns the renderer-specific LDrawTexture subclass to
//				instantiate. Falls back to LDrawTexture itself if no renderer
//				registered (useful for parser tests that don't actually draw).
//
//------------------------------------------------------------------------------
+ (Class)textureClass
{
	return LDrawTexture_registeredClass != Nil ? LDrawTexture_registeredClass
											   : [LDrawTexture class];
} // end textureClass


//---------- registerTextureClass: -----------------------------------[static]--
//
// Purpose:		Registers the renderer-specific subclass as the texture class.
//				Called from the renderer package's +load method.
//
//------------------------------------------------------------------------------
+ (void)registerTextureClass:(Class)cls
{
	LDrawTexture_registeredClass = cls;
} // end registerTextureClass:
