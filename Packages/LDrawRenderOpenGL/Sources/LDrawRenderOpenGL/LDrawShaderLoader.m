...
{
	if (file_path == nil) {
		printf("Shader load failed: nil file path.\n");
		return 0;
	}
	...
	if (shader_file_text == nil) {
		printf("Shader load failed: could not read %s.\n", [file_path UTF8String]);
...
GLuint	LDrawLoadShaderFromResource(NSString * name, const char * _Nullable const * _Nullable attrib_list)
{
	...
	if (path == nil) {
		printf("Shader load failed: %s not found in %s.\n",
			   [name UTF8String],
			   [[resourceBundle bundlePath] UTF8String]);
	}
