...
// Process-wide LSynth configuration provider. Set by the host app at
// launch with an LSynthConfiguration adapter (LDrawFeatures).
...
//---------- configSource -------------------------------------------[static]--
//
// Purpose:		Return the process-wide LSynth configuration provider installed
...
//
// Purpose:		Install the host's LSynth configuration adapter. LDrawCore
//				cannot depend on LDrawFeatures, so the host wires this at
//				startup.
//
//------------------------------------------------------------------------------
...
{
    id<LDrawLSynthConfigSource>config	= [LDrawLSynth configSource];
...
#if !TARGET_OS_OSX
    // External LSynth process launch is macOS-only (NSTask). Other hosts
    // provide synthesis through LDrawFeatures.
    (void)input;
    (void)configPath;
    (void)lsynthPath;
    return;
...
- (BOOL)acceptsDroppedDirective:(LDrawDirective *)directive
{
    ...
    if (	([directive isKindOfClass:[LDrawPart class]] && [[LDrawLSynth configSource] isLSynthConstraint:(LDrawPart*)directive])
