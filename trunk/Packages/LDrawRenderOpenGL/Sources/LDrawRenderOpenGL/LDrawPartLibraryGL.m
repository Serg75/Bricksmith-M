...
@implementation LDrawPartLibraryGL

static LDrawPartLibraryGL *SharedPartLibrary = nil;
...
+ (void)load
{
	[LDrawPartLibrary registerSharedPartLibrary:[LDrawPartLibraryGL sharedPartLibrary]];
} // end load
...
+ (LDrawPartLibraryGL *)sharedPartLibrary
{
	...
	{
		SharedPartLibrary = [[LDrawPartLibraryGL alloc] init];
	}
...
- (GLuint)textureTagForTexture:(LDrawTextureGL *)texture
{
	...
	else
	{
		...
		if (image)
		{
			...
			// glTexImage2D has copied the pixels into the texture object.
			free(imageBuffer);
		}
	}
