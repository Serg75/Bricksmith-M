...
@implementation LDrawPartReport
...
{
	LDrawPartReport *partReport = [LDrawPartReport new];
...
{
	LDrawPartLibrary *partLibrary		= [LDrawPartLibrary sharedPartLibrary];
...
//---------- previewPartFromRecord: ----------------------------------[static]--
//
// Purpose:		Makes the current part displayed match the part selected in the
//				table. We have to make sure to draw the optimized vertexes. The
//				easiest way to do that is to create a part referencing the
//				model.
//
//------------------------------------------------------------------------------
+ (LDrawPart *)previewPartFromRecord:(NSDictionary *)record
{
	...
	NSString   *partName  = [record objectForKey:PART_REPORT_NUMBER_KEY];
	LDrawColor *partColor = [record objectForKey:PART_REPORT_LDRAW_COLOR];
	if (partName == nil) return nil;
...
//---------- pieceCountSaveDialogTitleKey ----------------------------[static]--
//
// Purpose:		Piece-count export save-panel localization keys. The host still
//				localizes.
//
//------------------------------------------------------------------------------
+ (NSString *)pieceCountSaveDialogTitleKey
{
	return @"PieceCountSaveDialogTitle";
}


//---------- pieceCountSaveDialogMessageKey -------------------------[static]--
//
// Purpose:		Localization key for the piece-count save dialog message.
//				The host still localizes.
//
//------------------------------------------------------------------------------
+ (NSString *)pieceCountSaveDialogMessageKey
{
	return @"PieceCountSaveDialogMessage";
}


//---------- untitledLocalizationKey --------------------------------[static]--
//
// Purpose:		Localization key for an untitled document name. The host
//				still localizes.
//
//------------------------------------------------------------------------------
